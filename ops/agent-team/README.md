...
Governance-Schicht und ersetzt weder `AGENTS.md`, `00-schema/AGENTS.md`
noch die laufende Workstream-Discipline.
...
- `00-schema/AGENTS.md` ist die kanonische Function-Wiki- und Frontmatter-
  Spec. Diese Swarm-Contracts folgen ihr für `page_type`, `status`,
  `tags` und `updated`.
- `02-wiki/index.md` ist die kanonische Wiki-Index-Schicht mit
  `[[link]]`-Katalog. Neue dauerhafte Verweise werden dort eingetragen.
- `02-wiki/log.md` ist die Append-only-Log-Schicht. Erfolgreiche
  Aenderungen werden dort angehängt.
- `03-mspr/packets/` ist die kanonische Ablage für Risk- und Block-
  Packets. Neue Bootstrap-Pakete folgen dem bestehenden YAML-Format
  (siehe `2026-05-09-verification-gate.yaml` als Referenz).
...
1. Menschlicher Review (siehe `03-mspr/packets/2026-06-08-swarm-governance-bootstrap.yaml`).
2. Bestätigung, dass die Tier-Mappings (0-4) und Policy-Regeln mit
   `AGENTS.md` und der laufenden Workstream-Discipline konsistent sind.
