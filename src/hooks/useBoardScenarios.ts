...
export function useBoardScenarios(input: {
  ...
  const queryKey = ['reasoning', 'board-scenarios', input.referenceId, version, contextKey] as const;
  ...
  const query = useQuery({
    queryKey,
    queryFn: () => reasoningApi.boardScenarios({ referenceId: input.referenceId, context: input.context, version }),
