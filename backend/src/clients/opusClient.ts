...
export interface OpusJsonResult {
  model: string;
  rawText: string;
  parsed: unknown;
}
...
export async function opusCallJson(prompt: string, options: {
  model: string;
  timeoutMs: number;
}): Promise<OpusJsonResult> {
  const env = getEnv();
  ...
  try {
    ...
    return { model: options.model, rawText, parsed };
  } finally {
